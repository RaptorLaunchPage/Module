...
-- Get the first admin user for test data
DO $$
DECLARE
    admin_user_id uuid;
    team_id_1 uuid;
...
BEGIN
    ...
    -- Get team IDs
    SELECT id INTO team_id_1 FROM public.teams WHERE name LIKE '%Main%' LIMIT 1;
    SELECT id INTO team_id_2 FROM public.teams WHERE name LIKE '%Jr%' OR name LIKE '%Junior%' LIMIT 1;
    ...
    -- Get the tryout IDs
    SELECT id INTO tryout_id_1 FROM public.tryouts WHERE name = 'Raptors Main - July 2025';
    SELECT id INTO tryout_id_2 FROM public.tryouts WHERE name = 'Raptors Jr - New Team Formation';
    ...
    (
        ...
        '+1555666777',
        'Rising talent looking for first competitive team. Very dedicated and eager to learn.',
        'applied',
        now() - interval '1 hour'
    )
    RETURNING id INTO app_id_1;
    
    -- Get application IDs for further test data
    SELECT id INTO app_id_1 FROM public.tryout_applications WHERE ign = 'AlexGamer2024';
    SELECT id INTO app_id_2 FROM public.tryout_applications WHERE ign = 'SarahSniper';
    SELECT id INTO app_id_3 FROM public.tryout_applications WHERE ign = 'MikeR_Pro';
    ...
    INSERT INTO public.tryout_sessions (
        ...
        tryout_id_1,
        (SELECT id FROM public.tryout_invitations WHERE application_id = app_id_2),
        'scrim',
...
-- Your tryouts module is now ready to use!
-- 
-- Next steps:
-- 1. Visit /dashboard/tryouts to see the main dashboard
-- 2. Create a new tryout campaign
-- 3. Share the application link: /apply/[tryout-id]
-- 4. Review applications and manage the evaluation process
--
-- The module includes:
